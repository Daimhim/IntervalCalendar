...
class IntervalCalendarController(private val intervalCalendarAdapter: IntervalCalendar.IntervalCalendarAdapter) {
    ...
    fun init() {
        ...
        pager
            .liveData
            .observe(lifecycle) {
                intervalCalendarAdapter.submitData(lifecycle.lifecycle, it)
            }
        intervalCalendarAdapter.setOnItemClickListener { viewHolder, view, position ->
            ...
            val tmpStartTime = config.startTime
            val tmpFinishTime = config.finishTime
            // 当前时间
...
//            }
            ...
            if (startSameDay <= 0){
                ...
                config.startTime = itemDay.millis
                config.finishTime = -1
                intervalCalendarAdapter.chooseStatusChange(position,if (tmpFinishTime < 0) tmpStartTime else tmpFinishTime)
                onSelectTimeChangeListener?.onTimeChange(config.startTime,config.finishTime)
                return@setOnItemClickListener
            }
            // 从未选择结束时间
...
//            }
            ...
            if (endSameDay >= 0){
                ...
                config.startTime = itemDay.millis
                config.finishTime = -1
                intervalCalendarAdapter.chooseStatusChange(tmpStartTime,position)
                onSelectTimeChangeListener?.onTimeChange(config.startTime,config.finishTime)
                return@setOnItemClickListener
            }
            intervalCalendarAdapter.chooseStatusChange(config.startTime,config.finishTime)
            config.startTime = itemDay.millis
            config.finishTime = -1
            intervalCalendarAdapter.chooseStatusChange(position,-1)
            // 点击
            onSelectTimeChangeListener?.onTimeChange(config.startTime,config.finishTime)
        }
    }

    class EveryDaySource(val config : Config) : PagingSource<String, IntervalCalendar.EveryDay>() {
        ...
        override fun getRefreshKey(state: PagingState<String, IntervalCalendar.EveryDay>): String? {
        ...
        override suspend fun load(params: LoadParams<String>): LoadResult<String, IntervalCalendar.EveryDay> {
            val key = params.key!!
            val format = String.format("%s%s", key, ZERO_TIME)
            ...
            Log.i("EveryDaySource",
                String.format("prevKey:%s key:%s nextKey:%s  data:%s",
         ...
         */
        private fun getMonth(time: Long): MutableList<IntervalCalendar.EveryDay> {
            val mutableListOf = mutableListOf<IntervalCalendar.EveryDay>()
            ...
            // 标题
            mutableListOf.add(IntervalCalendar.EveryDay().also {
